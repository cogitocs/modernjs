...
var PieceworkWages = require('../../models/payroll').PieceworkWages;
var PieceworkRate = require('../../models/payroll').PieceworkRate;
...
var excel_json = function(req, res) {
	...
	async.series({
		...
		update_data: function(cb) {
			get_piecewagesToPeople_update(req, res, function(data) {
				cb(null, data)
			})
		}
	}, function(err, result) {
	...
	//将导入的数据更新到人员薪资表中
	function get_piecewagesToPeople_update(req, res, cb) {
		var client = req.user.client.id;
		async.waterfall([
				...
				function(result, cb) {
					...
					async.times(result.length, function(n, next) {
						...
						PayrollPeople.findOne({
							...
							var piece_wage_value = func_value(pep_piece[pep_info.people]);
			...
			function(err, results) {
				if (err) {
					req.app.locales.Handle500(err, req, res)
				}
				if (results) {
					cb(results)
				}
...
//生成JSON数据 
//导出EXCEL表
var get_pieceworks = function(req, res) {
	...
	async.series({
		...
		var fname = encodeURIComponent(result.company.company_name + '(计件数据).xls')
		...
		async.waterfall([
				...
				function(pis, cb) {
					...
					async.times(peps.length, function(n, next) {
						...
						obj.pep_pieceworks.push('件数');
						// obj.pep_pieceworks.push('计件工资费率区间名称');
						obj.pep_pieceworks.push('开始日期');
						obj.pep_pieceworks.push('结束日期');
						next(null, obj)
					}, function(err, datas) {
						var obj_data = {};
						obj_data.filename = "Piecework.xls";
						obj_data.worksheets = [];
						var py = {};
						py.ws_name = piece[0].pri ? piece[0].pri.pri_name : '计件工资';
						py.col_num = '10';
						py.data = [];
						var res_data;
...
var company_get_json_data = function(req, res) {
	var i18n = req.i18n;
	var client = req.user.client.id;
	Company.find({
		client: client,
		...
		return res.json({
			code: 'OK',
			msg: '公司代码数据获取成功，数据在data字段中。',
			data: tccs,
		});
	})
}
var pri_name_json_data = function(req, res) {
	var client = req.user.client.id;
	PayrollItemClient.find({
		client: client
		...
		us.each(result, function(temp) {
			item1.push(temp.id);
			item2.push(temp.pri_name)

		})
		var tccs = us.object(item1, item2);
		return res.json({
			code: 'OK',
...
module.exports = function(app, checkAuth) {
	...
	app.post(__base_path + '/s007', checkAuth, excel_json);
	app.get(__base_path + '/get_pieceworks/:company', checkAuth, get_pieceworks);
	app.get(__base_path + '/pri_name_json_data', checkAuth, pri_name_json_data);
	app.get(__base_path + '/company_get_json_data', checkAuth, company_get_json_data);
	app.get(__base_path + '/get_piecewagesToPeople_update', checkAuth, get_piecewagesToPeople_update)
}
                                                        data_collection by Ivan
